...
function openRemoteBrowsers () {
    var ConnectorCtor = isBrowserStack ? BsConnector : SlConnector;

    connector = new ConnectorCtor(environment[browserProvider].username, environment[browserProvider].accessKey);
    ...
    return connector
        ...
        .then(function () {
            var openBrowserPromises = browsersInfo.map(function (browserInfo) {
                return connector.startBrowser(browserInfo.settings, browserInfo.connection.url, environment.jobName);
            });
